...
export const useGuestCheckout = () => {
  ...
  const calculateCosts = useCallback(() => {
    ...
    const shippingCost =
      state.guestUser.province && cartTotalAmount > 0
    ...
    cartTotalAmount,
    state.currentOrder?.appliedCoupon,
    state.guestUser.province,
  ]);
  ...
  const updateGuestUser = useCallback(
    (field: keyof GuestUser, value: string) => {
      setState((prev) => ({
        ...prev,
  ...
  const createGuestOrder = useCallback(
    async (appliedCoupon?: AppliedCoupon, paymentMethod?: PaymentMethod) => {
  ...
  const processGuestPayment = useCallback(
    async (orderId: string) => {
      ...
      try {
        const response = await fetch(
          "/api/mercadopago/create-guest-preference",
          {
            method: "POST",
