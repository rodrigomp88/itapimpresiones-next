...
export const useSearch = () => {
  ...
  const getCacheKey = useCallback((queryText: string, filters: SearchFilters, sortBy: string) => {
  ...
  const getSuggestions = useCallback(async (searchTerm: string): Promise<string[]> => {
  ...
  }, [state.hasMore, state.loading, state.searchQuery, state.filters, state.sortBy, searchProducts]);
