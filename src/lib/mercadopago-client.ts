...
export class MercadoPagoClient {
    ...
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        if (!categorizedError.retryable || attempt === this.maxRetries) {
          throw new Error(`${operationName} failed after ${attempt} attempts: ${categorizedError.message}`);
        }

        // Calcular delay de reintento basado en el tipo de error
        const delay = this.getRetryDelay(categorizedError.type, attempt);
        
        console.log(`Retrying ${operationName} in ${delay}ms (attempt ${attempt + 1}/${this.maxRetries})`);
   ...
   */
  private getRetryDelay(errorType: MercadoPagoErrorType, attempt: number): number {
   ...
   */
  async getPayment(paymentId: string, metadata: Record<string, any> = {}): Promise<any> {
   ...
   */
  async createPreference(preferenceData: any, metadata: Record<string, any> = {}): Promise<any> {
  ...
  private validatePreferenceData(data: any): void {
    ...
    data.items.forEach((item: any, index: number) => {
      if (!item.title || !item.quantity || !item.unit_price) {
        throw new Error(`Item ${index + 1} is missing required fields (title, quantity, unit_price)`);
      }
   ...
   */
  async createRefund(paymentId: string, refundData: any = {}, metadata: Record<string, any> = {}): Promise<any> {
   ...
   */
  async getRefund(refundId: string, metadata: Record<string, any> = {}): Promise<any> {
    return this.executeWithRetry(
   ...
   */
  async cancelPayment(paymentId: string, metadata: Record<string, any> = {}): Promise<any> {
    return this.executeWithRetry(
    ...
    if (data.items) {
      data.items.forEach((item: any, index: number) => {
        ...
        if (!item.unit_price || typeof item.unit_price !== 'number' || item.unit_price <= 0) {
          errors.push(`Item ${index + 1}: unit_price is required and must be a positive number`);
        }
      });
    }
