...
const RelatedProducts: React.FC<RelatedProductsProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'related' | 'upselling' | 'bought_together'>('related');
  const { relatedProducts, upsellingProducts, boughtTogetherProducts, loading } = useRecommendations(currentProduct);
      ...
      <div className="p-6">
                      ...
                      {product.recommendation && (
                        <div className="absolute top-2 left-2">
                          <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                            ...
                            {product.recommendation.type === 'related' && 'Relacionado'}
                            {product.recommendation.type === 'upselling' && 'Alternativa'}
                            {product.recommendation.type === 'bought_together' && 'Popular'}
                          </span>
                        </div>
                      )}
                        ...
                        <div>
                          ...
                          {product.recommendation?.score && (
                            <div className="text-xs text-zinc-500 dark:text-zinc-400">
                              Match: {Math.round(product.recommendation.score * 100)}%
                            </div>
                          )}
                        </div>
        ...
        {currentProducts.length >= maxProducts && (
          <div className="mt-6 text-center">
            ...
            >
              <span>Ver todos los {tabs.find(tab => tab.id === activeTab)?.label.toLowerCase()}</span>
              <HiPlus className="w-4 h-4" />
            </Link>
          </div>
        )}
